...
REPO = 'mecha-trace'
PROJECT_TERMS = ['mecha', 'state', 'model', 'interpretable', 'features', 'interpretability', 'amerirad', 'rollout']
PROJECT_METRICS = ['mecha_coverage', 'state_risk', 'model_precision', 'interpretable_latency']
PROJECT_FAILURES = ['mecha_drift', 'state_gap', 'model_misroute', 'interpretable_blindspot']
PROJECT_ARCHETYPES = [{'name': 'mecha evidence replay', 'trigger': 'mecha signal changes while state context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'state boundary probe', 'trigger': 'state handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'model regression harness', 'trigger': 'model behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'interpretable operator packet', 'trigger': 'interpretable output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A feature attributed report review SDK and viewer: every sentence in a Mecha draft links to a named SAE feature, a saliency overlay on the image, and a one click accept/reject that re renders the report and writes an audit row to a verifiable log.'
...
HERO_TITLE = 'Mecha Health Evidence Triage Board'
